(function() {let __HMR_ID = "0.9re6ik580g6";
(function () {
  'use strict';
...
(function() {
  ...
  function checkStoragePermission(storageType) {
    ...
    if (chrome$1.storage[storageType] === void 0) {
  ...
  function createStorage(key, fallback, config) {
    ...
    if (globalSessionAccessLevelFlag === false && storageType === "session" && (config == null ? void 0 : config.sessionAccessForContentScripts) === true) {
      ...
      chrome$1 == null ? void 0 : chrome$1.storage[storageType].setAccessLevel({
    ...
    const _getDataFromStorage = () => __async$1(this, null, function* () {
      var _a2;
      checkStoragePermission(storageType);
      const value = yield chrome$1 == null ? void 0 : chrome$1.storage[storageType].get([key]);
      if (!value) {
        return fallback;
      }
    ...
    const set = (valueOrUpdate) => __async$1(this, null, function* () {
      cache = yield updateCache(valueOrUpdate, cache);
      yield chrome$1 == null ? void 0 : chrome$1.storage[storageType].set({ [key]: serialize(cache) });
      _emitChange();
    });
    ...
    if (liveUpdate) {
      chrome$1 == null ? void 0 : chrome$1.storage[storageType].onChanged.addListener(_updateFromStorageOnChanged);
    }
    return {
  ...
  const exampleThemeStorage = __spreadProps(__spreadValues({}, storage), {
    // TODO: extends your own methods
    toggle: () => __async(void 0, null, function* () {
      yield storage.set((currentTheme) => {
  ...
  const resolveFetch$3 = (customFetch) => {
    ...
    if (customFetch) {
      ...
      _fetch = (...args) => Promise.resolve().then(() => browser$3).then(({ default: fetch2 }) => fetch2(...args));
    } else {
      _fetch = fetch;
    }
  ...
  const browser$3 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
    __proto__: null,
    Headers: Headers$1,
    Request,
    Response: Response$1,
  ...
  const require$$0 = /* @__PURE__ */ getAugmentedNamespace(browser$3);
  ...
  class RealtimeClient {
    ...
    connect() {
      ...
      Promise.resolve().then(() => browser$2).then(({ default: WS }) => {
  ...
  const resolveResponse = () => __awaiter$6(void 0, void 0, void 0, function* () {
    if (typeof Response === "undefined") {
      return (yield Promise.resolve().then(() => browser$3)).Response;
    }
    return Response;
  });
  ...
  class GoTrueClient {
    ...
    constructor(options) {
      ...
      const settings2 = Object.assign(Object.assign({}, DEFAULT_OPTIONS), options);
      ...
      this.fetch = resolveFetch(settings2.fetch);
      this.lock = settings2.lock || lockNoOp;
  ...
  class SupabaseClient {
    ...
    constructor(supabaseUrl2, supabaseKey2, options) {
      ...
      const settings2 = applySettingDefaults(options !== null && options !== void 0 ? options : {}, DEFAULTS);
      this.storageKey = (_a = settings2.auth.storageKey) !== null && _a !== void 0 ? _a : "";
      this.headers = (_b = settings2.global.headers) !== null && _b !== void 0 ? _b : {};
      if (!settings2.accessToken) {
        this.auth = this._initSupabaseAuthClient((_c = settings2.auth) !== null && _c !== void 0 ? _c : {}, this.headers, settings2.global.fetch);
      } else {
      ...
      this.fetch = fetchWithAuth(supabaseKey2, this._getAccessToken.bind(this), settings2.global.fetch);
  ...
  chrome.storage.local.get("settings", function(data) {
    ...
    highlightArtiklID = (_a = data.settings) == null ? void 0 : _a.highlightArticle;
    if (highlightArtiklID) {
      sendSupaHighlightArticle(highlightArtiklID);
      chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
        if (tabs[0].id) chrome.tabs.sendMessage(tabs[0].id, { highlightArtiklID });
      });
    }
  });
    ...
    if (changeInfo.status === "complete") {
      chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
        if (tabs.length > 0 && tabs[0].id === tabId) {
          ...
          fetchPopisArtikala().then((popisArtikala) => {
            chrome.tabs.sendMessage(tabId, { popisArtikala });
            chrome.storage.local.set({ popisArtikala });
          }).catch((error) => {
            console.error("Greška pri dohvaćanju podataka o popisArtikala:", error);
          });
          fetchHighlightedArticleSupa().then((data) => {
            console.log(data);
  ...
  const fetchHighlightedArticleSupa = async () => {
    let { data: highlight, error } = await supabase.from("kdHighlightArticle").select("*");
  ...
  async function fetchNotifications() {
    const { data, error } = await supabase.from("kitchenNotifikations").select("*").eq("poslovnica", poslovnica).eq("isActive", true);
    if (error) {
  ...
  async function sendSupaHighlightArticle(highlightArticleId) {
    try {
  ...
  async function updateHighlightedArticle(highlightArticleId) {
    const { data, error } = await supabase.from("kdHighlightArticle").update({
      poslovnica,
      article_ID: highlightArticleId
    }).eq("poslovnica", poslovnica).select();
    console.log("123", error);
  }
  async function insertHighlightedArticle(highlightArticleId) {
    await supabase.from("kdHighlightArticle").insert([
      {
        poslovnica,
        article_ID: highlightArticleId
      }
  ...
  async function fetchTheme() {
    let { data: remarisMakeoverThemes, error } = await supabase.from("remarisMakeoverThemes").select("*");
    if (error) {
  ...
  async function savePopisArtikalaItemsToSupabase(popisArtikala) {
    try {
      const { data, error } = await supabase.from("artikliRemaris").insert(popisArtikala.map((item) => ({
  ...
  async function fetchPopisArtikala() {
    let { data, error } = await supabase.from("artikliRemaris").select("*");
    if (error) {
  ...
  chrome.runtime.onMessage.addListener(async (message, _sender, sendResponse) => {
    ...
    if (message.type === "SEND_ANSWERS") {
      ...
      const { error } = await supabase.from("kitchenNotifikations").update({ response: message.notification.response, procitano: true }).eq("id", message.notification.id).select();
      if (error) {
    ...
    if (message.ticketsUIzradi) {
      ...
      if (noveNarudzbe.length > 0) {
        sendActiveTickets(noveNarudzbe).then(() => {
          sendResponse({ success: true });
        }).catch((error) => {
    ...
    if (message.popisArtikala) {
      ...
      removeExistingArikles(popisArtikala).then(() => {
        sendResponse({ success: true });
      }).catch((error) => {
        console.error("Greška pri spremanju sortiranih stavki na Supabase:", error);
        sendResponse({ success: false });
  ...
  setInterval(checkForUpdates, 24 * 60 * 60 * 1e3);
